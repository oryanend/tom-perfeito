...
public class ControllerExceptionHandler implements AuthenticationFailureHandler {
  ...
  public ResponseEntity<StandardError> resourceAlredyExists(
  ...
  public ResponseEntity<StandardError> pathNotFound(
      PathNotFoundException e, HttpServletRequest request) {
    HttpStatus status = HttpStatus.NOT_FOUND;
    StandardError err = new StandardError();
    err.setTimestamp(Instant.now());
    err.setStatus(status.value());
  ...
  public ResponseEntity<StandardError> unauthorizedAction(
