...
public class MusicControllerTest {
  ...
  private MusicDTO withoutTitleMusicDTO,
      withoutDescriptionMusicDTO,
      withoutReleaseDateMusicDTO,
      withoutLyricMusicDTO;
  ...
  void findAllShouldReturnPagedList() throws Exception {
    ...
    result.andExpect(status().isOk()).andExpect(jsonPath("$.content").isArray());
  }
  ...
  @Test
  @DisplayName(
      "POST `/musics` should return 422 when trying to insert a music without release date")
  ...
  @Test
  @DisplayName("PATCH `/musics/{id}` should update music when is admin")
  void updateMusicWithAdminCredentials() throws Exception {
    // Get token user
    String registerUserAndObtainAcessToken = registerUserAndObtainAcessToken(validUserDTO);
    ...
    // Try to get token with the admin user
    String registerUserAndObtainAcessTokenAdminUser = registerUserAndObtainAcessToken(adminUserDTO);
    ...
    ResultActions result =
        mockMvc.perform(
            patch(musicUrl + "/" + existingId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonBody)
                .header("Authorization", "Bearer " + registerUserAndObtainAcessTokenAdminUser)
                .accept(MediaType.APPLICATION_JSON));
  ...
  void updateMusicWithInvalidCredentials() throws Exception {
    ...
    String registerUserAndObtainAcessTokenSecondUser =
        registerUserAndObtainAcessToken(secondValidUserDTO);
  ...
  void deleteMusicWithValidCredentials() throws Exception {
    ...
    ResultActions result =
        mockMvc.perform(
            delete(musicUrl + "/" + existingId)
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "Bearer " + registerUserAndObtainAcessToken)
                .accept(MediaType.APPLICATION_JSON));

    result.andExpect(status().isNoContent()).andExpect(jsonPath("$").doesNotExist());
  ...
  @Test
  @DisplayName("DELETE `/musics/{id}` should delete music when is admin")
  void deleteMusicWithAdminCredentials() throws Exception {
    // Get token user
    String registerUserAndObtainAcessToken = registerUserAndObtainAcessToken(validUserDTO);
